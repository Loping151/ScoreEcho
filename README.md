...
> 建议先安装 `XutheringWavesUID` 主插件再装本插件。本插件软依赖于 XutheringWavesUID（资源文件、别名表、`分析登录` 国际服登录命令、自动 UID 同步等都依赖它）；主插件未装时本插件仍可独立运行，但部分功能会缺失或退化为占位提示。
...
![image.png](examples/fx0.png)

![image.png](examples/fx1.png)
...
控制台配置项 `templates` 可指定评分图随机模板池，类型为列表字符串。可选值：

- `all`：使用全部模板随机，留空或包含 `all` 时也按全部模板随机
- `ribbon`
- `porcelain`
- `midnight`
- `scoreband`
...
示例：填写 `["porcelain", "legacy_dark"]` 时，只会在这两个模板中随机。

![score_templates.png](examples/score_templates.png)
